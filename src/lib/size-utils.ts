...
export function validateGptImage2Size(width: number, height: number): SizeValidation {
    if (!Number.isFinite(width) || !Number.isFinite(height) || width <= 0 || height <= 0) {
        return { valid: false, reason: '宽和高必须为正数。' };
    }
    if (!Number.isInteger(width) || !Number.isInteger(height)) {
        return { valid: false, reason: '宽和高必须为整数。' };
    }
    if (width % GPT_IMAGE_2_EDGE_MULTIPLE !== 0 || height % GPT_IMAGE_2_EDGE_MULTIPLE !== 0) {
        return { valid: false, reason: `边长必须为 ${GPT_IMAGE_2_EDGE_MULTIPLE} 的倍数。` };
    }
    if (width > GPT_IMAGE_2_MAX_EDGE || height > GPT_IMAGE_2_MAX_EDGE) {
        return { valid: false, reason: `最大边长为 ${GPT_IMAGE_2_MAX_EDGE} 像素。` };
    }
    const long = Math.max(width, height);
    const short = Math.min(width, height);
    if (long / short > GPT_IMAGE_2_MAX_ASPECT) {
        return { valid: false, reason: `长宽比(长:短)必须 ≤ ${GPT_IMAGE_2_MAX_ASPECT}:1。` };
    }
    const pixels = width * height;
    if (pixels < GPT_IMAGE_2_MIN_PIXELS) {
        return { valid: false, reason: `总像素数必须至少为 ${GPT_IMAGE_2_MIN_PIXELS.toLocaleString()}。` };
    }
    if (pixels > GPT_IMAGE_2_MAX_PIXELS) {
        return { valid: false, reason: `总像素数不得超过 ${GPT_IMAGE_2_MAX_PIXELS.toLocaleString()}。` };
    }
    return { valid: true };
}
