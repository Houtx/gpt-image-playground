        ...
        <div className='app-panel flex h-full min-h-[300px] w-full flex-col items-center justify-between gap-4 overflow-hidden rounded-lg p-4'>
                    ...
                    viewMode === 'grid' ? (
                        <div
                            ...
                            {imageBatch.map((img, index) => (
                                <div
                                    key={img.filename}
                                    className='relative aspect-square overflow-hidden rounded border border-border bg-muted/30'>
                                    <Image
                                        src={img.path}
                                        alt={`生成图片 ${index + 1}`}
                                        fill
            ...
            <div className='flex h-10 w-full shrink-0 items-center justify-center gap-4'>
                {showCarousel && (
                    <div className='flex items-center gap-1.5 rounded-md border border-border bg-muted p-1'>
                        <Button
                            ...
                            aria-label='显示宫格视图'>
                            <Grid className='h-4 w-4' />发送到编辑</Button>
                        {imageBatch.map((img, index) => (
                            <Button
                                key={img.filename}
                                variant='ghost'
                ...
                <Button
                    ...
                    className={cn(
                        'shrink-0 app-soft-button disabled:pointer-events-none disabled:opacity-50',
